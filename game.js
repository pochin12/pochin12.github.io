class Game {
    constructor() {
        ...
        this.mario.game = this; // Add this line to pass game reference to Mario
    ...
    async loadAssets() {
        ...
        // Spawn entities after assets are loaded
        this.spawnEntities();
    }
    ...
    playBackgroundMusic() {
        ...
        this.sounds.bgm.play().catch(err => console.log('Audio playback failed:', err));
    }
    ...
    getInput() {
        ...
        const input = {
            ...
            run: this.keys['shift'] || (this.gamepad && this.gamepad.buttons[2].pressed), // B button for running
            crouch: this.keys['s'] || this.keys['arrowdown'] || (this.gamepad && (this.gamepad.buttons[1].pressed || this.gamepad.axes[1] > 0.7)),
            spinJump: this.keys['r'] || (this.gamepad && this.gamepad.buttons[5].pressed) // Right bumper for spin jump
        };
    ...
    update(input, world, entities) {
        ...
        if (this.mario.x >= 10000 && !this.gameWon) {
            this.gameWon = true;
